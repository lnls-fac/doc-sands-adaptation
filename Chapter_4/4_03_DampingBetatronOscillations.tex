...
section, however, I am going to make the simplifying assumption that $\eta$ is a constant, so that $\eta' = 0$ (this is a reasonable approximation but including $\eta'$ in the calculation is not that difficult and it yields to the same approximated result, see appendix \ref{sec:hor_damping}); and write the variation of $x_\beta$ with $s$ in the same form that I took for $y$; namely,
\begin{align}
